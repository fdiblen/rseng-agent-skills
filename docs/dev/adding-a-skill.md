...
Three curated maps must know every skill. All of them fail loudly on a
missing entry, so the build tells you if you forget:

- `pipeline/src/rseng_pipeline/skill_directory.py`, `CLUSTERS` - which
  practice cluster the skill belongs to. Required.
- `pipeline/src/rseng_pipeline/skill_directory.py`, **either** `SIGNALS`
  **or** `PHASE_ONLY` - required, and mutually exclusive. Add a
  high-precision `SIGNALS` rule if some file in a project indicates the
  skill applies (rule names must be unique), or list the skill in
  `PHASE_ONLY` with a one-line reason why no file could indicate it. A
  skill in neither trips an assertion naming both escape routes.
- `pipeline/src/rseng_pipeline/related_skills.py`, `RELATED` - for each
  neighbouring skill, one short line saying when that neighbour becomes
  relevant. Add the reverse edges on the neighbours too, where they make
  sense.
...
Run what CI runs, or `just check` for the whole sequence at once:

```
uv run --directory pipeline python -m rseng_pipeline.skill_lint
npx -y skills-ref@0.1.5 validate skills/rseng-<name>/
...
cd installer && npm test && npx biome ci src tests
```

- `skill_lint` and `skills-ref validate` are what actually check your
  frontmatter. `claude plugin validate . --strict` validates the marketplace
  manifest and passes on a skill with a broken frontmatter, so it is not the
  gate it looks like.
- `token_budget` enforces the per-description and per-body limits; a slightly
  long `description` fails CI and nothing else warns you.
- `link_check` verifies every URL in your Learn more list. It runs on a weekly
  schedule, not on your pull request, so run it yourself.
- The skill count in `.claude-plugin/marketplace.json`, the README, and the
  docs is regenerated by `readme_skills` in step 3. Do not edit it by hand.
