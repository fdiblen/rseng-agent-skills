...
def pack_version(repo_root: Path) -> str:
    """The one version everything else is checked against.

    installer/package.json is the only one the release process already
    verifies (publish.yml refuses a tag that disagrees with it), so it is
    the source rather than one more copy to keep in step.
    """
    manifest = json.loads(
        (repo_root / "installer" / "package.json").read_text(encoding="utf-8")
    )
...
def build_agents_skills(
    repo_root: Path, context: dict, target_dir: Path, tree: Path | None = None
) -> list[Path]:
    ...
    this location natively. A Gemini extension reads skills/ instead, so
    the destination is overridable."""
    tree = tree or target_dir / ".agents" / "skills"
