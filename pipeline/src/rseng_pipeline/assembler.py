...
FRAGMENT_HEADER = (
    ...
    "     From RSQKit ({url}) by the EVERSE project and the RSQKit team,\n"
...
def assemble(
    ...
    learn_more = merge_curated(collect_all(pages), defaults, per_page)
    ...
    for page_id, record in sorted(pages.items()):
        cleaned = clean_body(record.body, tools)
        page_entries[page_id] = _page_entry(record, learn_more[page_id])
