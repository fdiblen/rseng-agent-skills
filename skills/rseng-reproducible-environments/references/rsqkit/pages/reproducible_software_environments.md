<!-- Generated file - do not edit.
     Source: pages/tasks/reproducible_software_environments.md @ 03a8352e0701acf6ae28a1f6c9069e9b2caf8e7e
     From RSQKit (the Research Software Quality Kit) by the EVERSE project and the RSQKit team (https://everse.software/RSQKit/reproducible_software_environments),
...
- **Containerised environments** - use containers to encapsulate software and its dependencies, ensuring the software runs consistently across different systems regardless of the underlying host operating system.
  - [Docker](https://www.docker.com/) - creates lightweight, isolated containers for packaging software and its dependencies.
  - [SingularityCE](https://sylabs.io/singularity/), [Apptainer](https://apptainer.org/) - focuses on containerisation for high-performance computing (HPC) and scientific computing.
  - [Docker Compose](https://docs.docker.com/compose/) - manages multi-container Docker applications, facilitating reproducible environments with multiple services.
- **System-level environments** - work at the system level, ensuring that the entire system, including the operating system and application configurations, is reproducible.
  - [Vagrant](https://www.vagrantup.com/) - creates reproducible virtualised environments using configuration scripts to define virtual machines.
  - [NixOS](https://nixos.org/) - ensures reproducible environments with declarative package management, tracking all system dependencies and configurations -
  ...
  - [Packer](https://www.packer.io/) - automates the creation of consistent machine images, supporting multiple platforms.
- **[Computational workflow environments](https://everse.software/RSQKit/computational_workflows)** - geared toward creating reproducible environments for scientific research involving
multi-step, multi-code complex data analysis pipelines (e.g. in bioinformatics):
  ...
  - Workflow management systems (WMSs), such as [Nextflow](https://www.nextflow.io), [Galaxy](https://galaxyproject.org/),
    [Snakemake](https://snakemake.readthedocs.io), [Apache Airflow](https://airflow.apache.org/) or [Parsl](https://parsl.readthedocs.io/en/stable/userguide/workflows/workflow.html), create execution environments and run computational workflows based on their definitions.
...
- There are often multiple package and environment management tools even for a single programming language:
  - For example, commonly used tools for managing Python packages and virtual environments are [pip](https://pypi.org/project/pip/) (Python package manager tool which interacts and obtains the packages
  from the central repository called [Python Package Index (PyPi)](https://pypi.org/)) and [venv](https://docs.python.org/3/library/venv.html) (Python virtual environment manager tool available by default from the standard Python distribution from Python 3.3).
  One alternative is to use [Poetry](https://python-poetry.org/) - a modern Python packaging tool which also installs Python packages from PyPI and handles virtual environments automatically.
  Also check [uv](https://github.com/astral-sh/uv) - a single and fast Python package and project manager, built to replace [pip](https://pypi.org/project/pip/) and [venv](https://docs.python.org/3/library/venv.html).
  - If your Python code relies on non-Python packages, for instance when some C++ libraries must also be installed and you want to support multiple platforms, a better choice may be [Conda](https://anaconda.org/anaconda/conda) -
  ...
  [Conda](https://anaconda.org/anaconda/conda) has its own repository system separate from (but compatible with) PyPI that distributes non-Python packages packages as well and has its own non-venv-based virtual environment system.
  - If you are using R - consider [renv](https://rstudio.github.io/renv/index.html) that will help you build reproducible environments for your R projects
  - For Julia programming language - check [Pkg.jl](https://pkgdocs.julialang.org/v1/); for C++ - check [Conan](https://conan.io/), for Java - check [Maven](https://maven.apache.org/), for Ruby - check [Bundler](https://bundler.io/).
  - There are some some generic tools to have a look at as well - e.g. [Spack][spack], [NixOS](https://nixos.org/), [guix][guix].
