<!-- Generated file - do not edit. Derived by the rseng-agent-skills
     pipeline from this skill's own curated links in SKILL.md. -->

# References

Based on community migration practice between scientific computing platforms and the linked ecosystem documentation.

Learn more:

- https://octave.org - GNU Octave
- https://numpy.org/doc/stable/user/numpy-for-matlab-users.html - NumPy for MATLAB users (official mapping guide)
- https://scipy.org - SciPy ecosystem
- https://github.com/blink1073/oct2py - oct2py Octave-Python bridge
- https://www.sympy.org - SymPy symbolic mathematics
- https://julialang.org - Julia
- https://pandas.pydata.org - pandas
- https://docs.astropy.org/en/stable/ - Astropy (the IDL-exodus ecosystem)
