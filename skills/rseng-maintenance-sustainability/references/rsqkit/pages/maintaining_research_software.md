<!-- Generated file - do not edit.
     ...
     From RSQKit (the Research Software Quality Kit) by the EVERSE project and the RSQKit team (https://everse.software/RSQKit/software_maintenance),
...
- **Write a thorough test suite and check your test coverage.** Use a coverage tool (e.g. [pytest-cov](https://pytest-cov.readthedocs.io/) for Python, [covr](https://covr.r-lib.org/) for R) to understand how much of your code is exercised by tests, and identify gaps.
- **[Set up a continuous integration (CI) pipeline](https://everse.software/RSQKit/ci_cd)** that runs your test suite automatically on every commit and on a scheduled basis (e.g. weekly).
  Scheduled runs catch breakage caused by external changes even when you are not actively developing.
  Tools such as [GitHub Actions](https://github.com/features/actions) or [GitLab CI/CD](https://docs.gitlab.com/ee/topics/build_your_application.html) make this straightforward.
- **Pin and manage your dependencies explicitly.** Use a [dependency management tool appropriate to your language](https://everse.software/RSQKit/reproducible_software_environments) (e.g. [pip](https://pypi.org/project/pip/), [venv](https://docs.python.org/3/library/venv.html), [pip-tools](https://pip-tools.readthedocs.io/en/latest/), [uv](https://github.com/astral-sh/uv) or [Poetry](https://python-poetry.org/) for Python, [renv](https://rstudio.github.io/renv/index.html) for R) to record exact versions and make updates deliberate and auditable.
  Consider using [Dependabot](https://github.com/dependabot/dependabot-core) or [Renovatebot](https://docs.renovatebot.com/) to automate dependency update pull requests.
- **[Keep your documentation current](https://everse.software/RSQKit/documenting_software_project).** Review your [README](https://everse.software/RSQKit/creating_good_readme), installation guide, and usage examples whenever you make a substantive change to the software. If you cannot install and run your own software from scratch using only your documentation, it needs updating.
- **Establish a clear issue tracker.** Use GitHub Issues, GitLab Issues, or an equivalent to record known bugs, planned improvements, and technical debt. This makes the [maintenance backlog visible and shared](https://everse.software/RSQKit/organising_software_projects), rather than held only in one person's head.
- **Use [Semantic Versioning](https://semver.org/)** to communicate the nature of changes to users. Following Semantic Versioning conventions (MAJOR.MINOR.PATCH) signals whether an update is a breaking change, a new feature, or a bug fix — helping users decide when and whether to upgrade. See also [Releasing software](https://everse.software/RSQKit/releasing_software).
- **Write a CHANGELOG** and update it with each [release](https://everse.software/RSQKit/releasing_software). A clear record of what changed, when, and why is valuable both for your users and for your future self trying to understand the history of the software.
- **Reduce the bus factor.** If you are the sole maintainer, document the key decisions, architecture, and operational knowledge in the repository itself — not just in your head. If possible, share ownership with at least one other person who can act if you are unavailable. Regular [code review](https://everse.software/RSQKit/code_review) is one effective way to spread knowledge of the codebase.
...
- **Use your issue tracker to record known technical debt.** Create issues (or a label/tag such as `tech-debt`) for known problems, shortcuts, and areas that need refactoring.
  This makes the debt visible to everyone working on the project and allows it to be prioritised alongside other work. See [Organising software projects](https://everse.software/RSQKit/organising_software_projects) for guidance on structuring your repository and issue tracker.
- **Add `TODO` and `FIXME` comments in code** at the point where debt exists, with enough context for a future reader to understand the problem.
  Consider using a linting tool to surface these systematically. See [Documenting code](https://everse.software/RSQKit/documenting_code) for broader guidance on keeping code understandable.
...
- **Refactor incrementally, not all at once.** Large-scale rewrites are high risk.
  Prefer small, targeted improvements — renaming for clarity, extracting a function, adding a missing test — that can be reviewed and validated individually. Use [code review](https://everse.software/RSQKit/code_review) to catch regressions and spread understanding of the changes.
- **Use static analysis tools** to surface code quality issues automatically.
  Tools like [Ruff](https://docs.astral.sh/ruff/) (Python), [lintr](https://lintr.r-lib.org/) (R), or [SonarQube](https://www.sonarcatalog.com/products/sonarqube/) can flag common problems and track quality metrics over time.
